...
def remove_set(current_set_name):
    ...
    if current_set_name != "":
        ...
        for obj in bpy.data.objects:
            set_list = obj.set_details.set_name.split(',')

            if current_set_name in set_list:
                index = set_list.index(current_set_name)

                # Select Active object in list
                active_obj = obj.set_details.active_obj.split(',')
                del active_obj[index]

                obj.set_details.active_obj = ','.join(active_obj)
                set_list.remove(current_set_name)
                obj.set_details.set_name = ','.join(set_list)
...
def add_selection_set(self, name):
    ...
    if name in enum_counter:
        for obj in selection_list:
            ...
            if name not in obj.set_details.set_name.split(','):
                obj.set_details.set_name = obj.set_details.set_name + name + ','
        ...
        if name != "" and len(selection_list) != 0 and not re.search(",", name):
            ...
            for obj in selection_list:
                ...
                # Add Custom Property to each obj: Also check it active obj
                obj.set_details.set_name = obj.set_details.set_name + name + ','

                if obj == active_obj:
                    obj.set_details.active_obj = obj.set_details.active_obj + "True" + ','
                else:
                    obj.set_details.active_obj = obj.set_details.active_obj + "False" + ','
        else:
...
def display_selection_name(self):
    ...
    for obj in selection_list:
        if obj.set_details.set_name != '':
            set_names.append(set(obj.set_details.set_name.split(',')))
        else:
            return string_name
    ...
    if len(set_names) > 1:
        ...
        for property in intersect:
            ...
            object_list = []
            for obj in bpy.data.objects:
                if property in obj.set_details.set_name.split(','):
                    obj_list.append(obj)
